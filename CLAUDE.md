...
- `/memory-bank/architecture/enrichment-reference.md` – Quick reference for LLM enrichment operations (read when working with restaurant data)
...
### Enrichment System
The enrichment system uses LLMs to discover and enhance restaurant data from web sources. It's a service-based architecture with clear separation of concerns.

**Key Components:**
- **Services** - Single-purpose business logic (restaurant enrichment, episode discovery, status verification)
- **Repositories** - Data access layer (restaurant, episode, city)
- **Workflows** - Multi-step orchestration (refresh stale data, status sweep, partial update, manual addition)
- **Shared Utilities** - LLM client, token tracker, result parser, retry handler

**When to Use:**
- Adding new restaurants: Use `workflows.manualRestaurantAddition`
- Refreshing old data: Use `workflows.refreshStaleRestaurant`
- Checking restaurant status: Use `workflows.restaurantStatusSweep`
- Backfilling episodes: Use `enrichEpisodesOnly` or `workflows.partialUpdate` with mode 'episodes'
